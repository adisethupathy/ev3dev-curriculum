...
Authors: David Fisher and Ryan Taylor.
...
def main():
    ...
    forward_button['command'] = lambda state: forward(mqtt_client, left_speed, right_speed)
    root.bind('<Up>', lambda event: forward(mqtt_client, left_speed, right_speed))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    left_button['command'] = lambda: left(mqtt_client, left_speed)
    ...
    right_button['command'] = lambda: right(mqtt_client, right_speed)
    root.bind('<Right>', lambda event: right(mqtt_client, right_speed))
    # right_button and '<Right>' key

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    back_button['command'] = lambda: back(mqtt_client, left_speed, right_speed)
    root.bind('<Down>', lambda event: back(mqtt_client, left_speed, right_speed))
